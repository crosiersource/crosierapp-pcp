<template>
  ...
  <CrosierFormS @submitForm="this.submitForm" titulo="Cliente">
    ...
    <div class="row">
      <CrosierInputEmail id="email" col="3" v-model="this.fields.email" />
    </div>
    ...
    <hr />
    <div class="row">
      <CrosierInputTextarea id="obs" label="Obs" v-model="this.fields.obs" />
    </div>
  </CrosierFormS>
</template>
...
export default {
  ...
  methods: {
    ...
    async buscarProxCodigo() {
      this.setLoading(true);
      try {
        const rs = await axios.get("/crm/cliente/findProxCodigo");
        this.fields.codigo = rs?.data?.DATA;
      } catch (e) {
        console.error(e);
      }
      this.setLoading(false);
